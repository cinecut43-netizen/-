...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (method === 'GET' && !action) {
      const { cat, city, status, limit = 50, offset = 0 } = req.query;
      // Раньше employer_id для "мои заказы" брался как есть из строки
      // запроса — целиком со стороны браузера (Shabashka.ensureDbUserId()).
      // Если по каким-то причинам браузер на секунду не смог подтвердить
      // серверную сессию (например сразу после обновления страницы) и
      // временно вернул НЕ тот id (например id=1 по умолчанию или id
      // только что созданной "запасной" учётки), кабинет работодателя
      // молча показывал чужой — точнее, ПУСТОЙ — список: реальные заказы
      // были на месте в базе, просто под другим employer_id их искали.
      // Теперь, если у запроса есть настоящая серверная сессия (cookie),
      // "мои заказы" всегда означают заказы ИМЕННО этого сессионного
      // пользователя — параметр employer_id из строки запроса для этого
      // применяется только когда сессии нет вовсе (публичный просмотр).
      const employer_id = req.authUserId || req.query.employer_id;
      let sql = `
